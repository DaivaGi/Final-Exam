...
import lt.techin.exception.FoodServiceDisabledException;
...
public class ApiExceptionHandler {
    ...
    @ExceptionHandler(FoodServiceValidationException.class)
    public ResponseEntity<ErrorDto> handleFoodServiceValidationException(HttpServletRequest request, FoodServiceValidationException foodServiceValidationException) {
        logger.error("FoodServiceValidationException: {}, for field: {}", foodServiceValidationException.getMessage(), foodServiceValidationException.getField());

        var errorStatus = HttpStatus.BAD_REQUEST;

        var errorFields = List.of(
                new ErrorFieldDto(foodServiceValidationException.getField(), foodServiceValidationException.getError(), foodServiceValidationException.getRejectedValue())
        );

        var errorDto = new ErrorDto(request.getRequestURL().toString(),
                errorFields,
                foodServiceValidationException.getMessage(),
                errorStatus.value(),
                errorStatus.getReasonPhrase(),
                request.getRequestURL().toString(),
                LocalDateTime.now());
    ...
    @ExceptionHandler(FoodServiceDisabledException.class)
    public ResponseEntity<Void> handleFoodServiceDisabledException(HttpServletRequest request, FoodServiceDisabledException serviceDisabledException) {
        logger.error("FoodServiceDisabledException: {}", serviceDisabledException.getMessage());
